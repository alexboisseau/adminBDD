...
J'ai ensuite ajouté un autre panel qui représente le nombre de connexions qui ont été refusées sur la base de données (j'ai volontairement essayé de me connecter avec les mauvais identifiants bien sûr). Pour ces données j'ai utilisé la metrics suivantes `mysql_global_status_access_denied_errors`.

![Nombre de connexion refusées sur la base de données "db"](./images/refused-connection.png?raw=true)

Pour cette première partie il reste un dernier panel à ajouter. Celui du temps nécessaire à l'exporter pour scrapper les données liées aux connections depuis le serveur MariaDB.

Metrics utilisée : `mysql_exporter_collector_duration_seconds{collector="collect.global_variables"}`

![temps nécessaire à l'exporter pour scrapper les données liées aux connections depuis le serveur MariaDB](./images/time-duration.png?raw=true)
